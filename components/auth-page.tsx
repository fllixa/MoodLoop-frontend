...
export function AuthPage() {
  ...
  // Email validation: must contain '@' and end with a valid domain (supports common TLDs)
  const validateEmail = (email: string) => {
    // Check for @ symbol and a valid domain with common TLDs
    const regex = /^[^\s@]+@[^\s@]+\.(com|org|net|edu|gov|co\.uk|io|dev|app)$/i;
    return regex.test(email);
  };

  // Specific validation function for login: checks for '@' and '.com' as requested
  const validateEmailForLogin = (email: string) => {
  ...
  const validateForm = (): boolean => {
    ...
    } else if (authMode === "login" && !validateEmailForLogin(email)) {
      newErrors.email = language === "en"
        ? "Email must contain '@' and end with '.com'"
        : "يجب أن يحتوي البريد الإلكتروني على '@' وينتهي ب '.com'";
      invalidFields.push("email");
    } else if (authMode === "signup" && !validateEmail(email)) {
  ...
  const handleSendResetLink = () => {
    ...
    // Transition to next step after 2 seconds
    setTimeout(() => {
                          ...
                          >
                            <div className="flex justify-center">
                              <motion.div
                                ...
                                className="w-10 h-10 border-2 border-[#614EA9] border-t-transparent rounded-full"
                              />
                            </div>
                            <p className="text-sm font-medium text-[#614EA9]">
                              {language === "en"
